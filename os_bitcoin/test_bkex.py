...
BASE_API_HOST2 = "https://api.bkex.vip"
...
if __name__ == "__main__":
    ...
    access_key = "vmsUZE6mv9SD5VNak4HlWFsOr6aKE2zvsw0MuIgwCIGy6utIco14y7Ju91duEh82"
    start = BkexAPI(secret_key,access_key)
    ticker = start.get_24hr_ticker_price_change_statistics(pair='BKK_USDT')
    for t in ticker:
        if type(ticker[t]) == dict:
            for tt in ticker[t]:
                print(tt, ticker[t][tt])
        else:
            print(t, ticker[t])

    bkex_ticker_dic = {'code':'',
                       'msg':'',
                       'data':{
                           'a':'',
                           'c':'',
